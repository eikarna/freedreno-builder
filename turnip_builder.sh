...
EOF
	...
	meson build-android-aarch64 --cross-file "$workdir"/mesa-main/android-aarch64 -Dbuildtype=release -Dplatforms=android -Dplatform-sdk-version=$sdkver -Dandroid-stub=true -Dgallium-drivers=freedreno,zink,virgl -Dvulkan-drivers=freedreno -Dvulkan-beta=true -Dfreedreno-kmds=kgsl,msm -Db_lto=true -Dopengl=true -Degl=true -Dosmesa=true -Dvulkan-layers=device-select -Dshader-cache-default=false -Dshader-cache-max-size=9999G -Dtools=freedreno -Dshader-cache=true -Dpower8=true &> "$workdir"/meson_log
